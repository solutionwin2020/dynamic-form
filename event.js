// https://imagelink.click/agXO.txt
function makeid() {
    var text = "";
...
$(document).ready(function(){
  ...
  $(".add_input_attribute").click(function(){
    ...
    $('.input_lbl_title').removeClass("_error");
    // .replace(/ /g,"_")
    var lbl_attr = clean($('.input_lbl_title').val());
    if(lbl_attr !== '')
    {
            ...
            {
              	var unq_id = makeid();
            	$(".custom_dynamic_form").append('<div class='+value+'><input name="'+lbl_attr+"__"+makeid()+'" type="checkbox" id='+unq_id+' value="'+lbl_attr+'" '+req+' /><label for='+unq_id+'></label></div>');    
            }
            else if(value == "textarea")
            {
  ...
  $(".add_radio_attribute ").click(function(){
    ...
    if(radio_val === ''){
        ...
        $(".custom_dynamic_form").append('<div class='+value+'><input name="" type="radio" id='+unq_id+"_"+radio_val+' '+req+' value="'+clean(radio_val)+'" /><label for='+unq_id+"_"+radio_val+' ></label></div>');
      ...
      if(u_key !== ""){
          $.ajax({
                ...
                data: {template_key:u_key}
                 ...
                 $(".custom_spinner").addClass("hide_me");
                 $(".custom_form_response").find("form").remove();
                if(response == 0){
  ...
  $(document).on("submit","form",function(event){
    event.preventDefault();
    var body = $("html, body");
    body.stop().animate({scrollTop:0}, 500, 'swing', function() {  // scroll to top
    });
    $(".custom_spinner").show();
    $(".success").empty();
    var form_data = {};
    try{
      form_data["form_data_template_key"] = $.trim($(".form_data_template_key").val());
      $("#dynamic_form").find("input").each(function(){
    ...
    if(form_data != null && form_data != undefined){
        ...
        type: "post",
        cache: 'false',
        data: {form_data_template_key:form_data}
        }).done(function(response, textStatus, jqXHR){
            $(".custom_spinner").hide();
            if(window.location.href.indexOf('update-form.php?edit_temp_id') > -1){
                window.location.href = window.location.origin+"/dynamic-form/form-data.php?key="+form_data["form_data_template_key"]+"&updated=update";
            }else{
                $(".success").attr("style","width: 55%;margin: auto;");
                $(".success").append('<h3 style="color:green;text-align:center;">Data Submit Successfully</h3><a href="'+window.location.origin+'/dynamic-form/form-data.php?key='+$.trim(response)+'" target="_blank">'+window.location.origin+'/dynamic-form/form-data.php?key='+$.trim(response)+'</a>');
            }
        }).fail(function (jqXHR, textStatus, errorThrown){
            $(".custom_spinner").hide();
            $(".success").append('<h3 style="text-align:center">Error: Data not submitted successfully</h3>');
...
function clean(str) {
    return str.replace(/[^\w\s]/gi, '_').replace(/ /g,"_") // Removes special chars.
}  
        ...
        data: {all_templates:"all"}
           ...
           $(".custom_spinner").addClass("hide_me");
            // setTimeout(function(){
                ...
                $(".templates_section .table tbody").append('<tr class="tbl_head_temp"><th>Unique Key</th><th>Template Name</th><th>Template URL</th><th></th><th></th></tr>');
                for(var i=0; i<length; i++){
                    var unique_key = Object.keys(response)[i];
                    var template_name = Object.keys(response[unique_key])[0];
                    $(".templates_section .table tbody").append('<tr><th>'+unique_key+'</th><td>'+template_name+'</td><td><a href="'+window.location.origin+'/dynamic-form/form.php?key='+unique_key+'" target="_blank">Click here to visit form</a></td><td><button class="edit_temp btn btn-primary custom_btn">Edit Template</button></td><td><button class="delete_temp btn btn-danger custom_btn" type="button" data-toggle="modal" data-target="#exampleModalCenter">Delete Template</button></td></tr>');
                }
            // },300)
